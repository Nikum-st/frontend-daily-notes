...
##Answer
Прототипное наследование — это механизм, при котором объект может использовать свойства и методы другого объекта через цепочку прототипов ([[Prototype]] или **proto**).

В JavaScript каждый объект имеет скрытое свойство [[Prototype]], которое указывает на другой объект — его прототип.

Это позволяет реализовать наследование без классов.
